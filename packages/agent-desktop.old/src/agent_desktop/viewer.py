...
from .systemd import CommandRunner, Runner
...
class ViewerServer:
    def __init__(
        ...
        self.manager = manager or SessionManager(config, config_path)
        ...
        self.app.add_routes(
            [
                ...
                web.get("/", self.index),
                web.get("/app.js", self.app_js),
                web.get("/style.css", self.style_css),
            ]
        )
        self.app.router.add_static(
            "/novnc/", str(config.novnc_web_root), follow_symlinks=False
        )
    ...
    async def sessions(self, request: web.Request) -> web.Response:
        ...
        return web.json_response(
            [self._viewer_session(session) for session in sessions]
        )

    async def destroy(self, request: web.Request) -> web.Response:
        self._authorize(request)
        session_id = self._session_id(request)
        session = await asyncio.to_thread(self.manager.destroy, session_id)
        return web.json_response(self._viewer_session(session))

    async def websocket(self, request: web.Request) -> web.StreamResponse:
        self._authorize(request)
        session_id = self._session_id(request)
        session = await asyncio.to_thread(self.manager.status, session_id)
        socket_path = self._validated_vnc_socket(session)
        ...
        try:
            async with (
                ClientSession(connector=connector) as client,
                client.ws_connect(
                    "ws://localhost/", protocols=("binary",), max_msg_size=0
                ) as upstream,
            ):
                tasks = {
    ...
    async def app_js(self, _request: web.Request) -> web.FileResponse:
        return self._asset("app.js")

    async def style_css(self, _request: web.Request) -> web.FileResponse:
        return self._asset("style.css")

    def _asset(self, name: str) -> web.FileResponse:
        asset = files("agent_desktop").joinpath("assets", name)
        return web.FileResponse(str(asset))
    ...
    def _validated_vnc_socket(self, session: Session) -> Path:
        expected = self.config.runtime_dir(session.id) / "wayvnc.sock"
        if not session.state.launchable:
            raise web.HTTPConflict(text=f"session is {session.state}")
        if session.vnc_socket != expected or not expected.is_socket():
            raise web.HTTPConflict(text="session VNC endpoint is unavailable")
        return expected

    @staticmethod
    def _viewer_session(session: Session) -> dict[str, object]:
        return {
            "id": session.id,
            ...
            "viewer_available": session.vnc_socket is not None
            and session.state.launchable,
        }
...
async def serve(config: Config, config_path: Path) -> None:
    ...
    await runner.setup()
    site = web.TCPSite(runner, "127.0.0.1", config.viewer_port)
    await site.start()
...
def command(config_path: Path) -> None:
    """Serve the local multi-session noVNC viewer shell."""
    try:
